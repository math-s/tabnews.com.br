...
export default function UsersPage({ userListFound, pagination }) {
  return (
    <DefaultLayout
      ...
      <UserList userList={userListFound} pagination={pagination} paginationBasePath="/moderacao/usuarios" />
    </DefaultLayout>
  );
}
...
export const getServerSideProps = async (context) => {
  ...
  if (secureUserListFound.length === 0 && context.params.page !== 1) {
    const lastValidPage = `/moderacao/usuarios/${results.pagination.lastPage || 1}`;
